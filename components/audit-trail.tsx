  ...
  const handleExportPDF = () => {
    ...
    if (workflow?.extracted.length) {
      ...
      workflow.extracted.forEach((ext) => {
        ...
        lines.push(
          `  Organization: ${ext.keyInformation.organization || "N/A"}`,
        );
        lines.push(`  Entities Found:`);
        lines.push(`    Names: ${ext.entities.names.join(", ") || "None"}`);
        lines.push(
          `    Organizations: ${ext.entities.organizations.join(", ") || "None"}`,
        );
        lines.push(
          `    Locations: ${ext.entities.locations.join(", ") || "None"}`,
        );
        lines.push("");
      });
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <Card className="p-8 text-center mb-6 bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
        ...
        <h4 className="text-xl font-bold text-green-700 dark:text-green-200 mb-2">
          Verification Complete
        </h4>
        <p className="text-green-600 dark:text-green-300">
          All documents have been processed and verified. You can now export the
          report or start a new verification.
        </p>
      </Card>
        ...
        <Card className="p-6 mb-6">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {workflow?.auditTrail.map((audit, idx) => (
              <div
                key={audit.id}
                className="flex gap-4 text-sm pb-3 border-b border-border last:border-b-0"
              >
